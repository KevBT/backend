...
route.get('/api/userdata', (req, res) => {
	...
	const sql = "select * from users INNER JOIN avatars ON users.avatar_id = avatars.id_avatar WHERE nickname = ?"
...
route.get('/api/game/listmessages/:id', (req, res) => {
	...
	const sql = "select posts.post, posts.date, users.nickname, avatars.image from posts INNER JOIN users ON users.id = posts.user_id INNER JOIN avatars ON  id_avatar = users.avatar_id WHERE game_id = ?"
